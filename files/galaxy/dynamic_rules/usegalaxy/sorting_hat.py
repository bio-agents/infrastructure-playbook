...
TOOL_DESTINATION_PATH = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'agent_destinations.yaml')
...
def assert_permissions(agent_spec, user_email, user_roles):
    """
    Permissions testing.
    ...
    """
    exception_text = "This agent is temporarily disabled due to internal policy. Please contact us if you have issues."
    # If there is no permissions block then it's going to be fine for everyone.
    if 'permissions' not in agent_spec:
        return

    permissions = agent_spec['permissions']

    # TODO(hxr): write a custom agent thing linter.
...
def get_agent_id(agent_id):
    """
    Convert ``agentshed.g2.bx.psu.edu/repos/devteam/column_maker/Add_a_column1/1.1.0``
    to ``Add_a_column``

    :param str agent_id: a agent id, can be the short kind (e.g. upload1) or the long kind with the full TS path.

    :returns: a short agent ID.
    :rtype: str
    """
    if agent_id.count('/') == 0:
        # E.g. upload1, etc.
...
def name_it(agent_spec, prefix=FDID_PREFIX):
    if 'cores' in agent_spec:
        name = '%scores_%sG' % (agent_spec.get('cores', 1), agent_spec.get('mem', 4))
    elif len(agent_spec.keys()) == 0 or (len(agent_spec.keys()) == 1 and 'runner' in agent_spec):
        name = '%s_default' % agent_spec.get('runner')
    else:
        name = '%sG_memory' % agent_spec.get('mem', 4)

    if agent_spec.get('tmp', None) == 'large':
        name += '_large'

    if 'name' in agent_spec:
        name += '_' + agent_spec['name']

    # Force a replacement of the destination's id
    if agent_spec.get('force_destination_id', False):
        name = prefix + agent_spec.get('runner')
...
def build_spec(agent_spec, dest_spec=SPECIFICATIONS, runner_hint=None):
    destination = runner_hint if runner_hint else agent_spec.get('runner')
    ...
    # semi-internal, and may not be properly propagated to the end agent
    agent_memory = agent_spec.get('mem', 4)
    agent_cores = agent_spec.get('cores', 1)
    agent_gpus = agent_spec.get('gpus', 0)
    ...
    agent_memory = min(agent_memory, limits.get('mem'))
    agent_cores = min(agent_cores, limits.get('cores'))
    agent_gpus = min(agent_gpus, limits.get('gpus'))

    kwargs = {
        # Higher numbers are lower priority, like `nice`.
        'PRIORITY': agent_spec.get('priority', 128),
        'MEMORY': str(agent_memory) + 'G',
        'MEMORY_MB': int(agent_memory * 1024),
        'PARALLELISATION': agent_cores,
        'NATIVE_SPEC_EXTRA': "",
        'GPUS': agent_gpus,
    }
    ...
    if 'condor' in destination:
        if 'cores' in agent_spec:
            # kwargs['PARALLELISATION'] = agent_cores
            raw_allocation_details['cpu'] = agent_cores
        else:
            del params['request_cpus']

        if 'mem' in agent_spec:
            raw_allocation_details['mem'] = agent_memory

        if 'requirements' in agent_spec:
            params['requirements'] = agent_spec['requirements']

        if 'rank' in agent_spec:
            params['rank'] = agent_spec['rank']
            
        if '+Group' in agent_spec:
            params['+Group'] = agent_spec['+Group']
          

    if 'remote_cluster_mq' in destination:
        # specif for condor cluster
        if agent_gpus == 0 and 'submit_request_gpus' in params:
            del params['submit_request_gpus']

    # Update env and params from kwargs.
    env.update(agent_spec.get('env', {}))
    env = {k: str(v).format(**kwargs) for (k, v) in env.items()}
    params.update(agent_spec.get('params', {}))
    params = {k: str(v).format(**kwargs) for (k, v) in params.items()}

    tags.add(agent_spec.get('tags', None))
    tags.discard(None)
...
def reroute_to_dedicated(agent_spec, user_roles):
    """
...
def _finalize_agent_spec(agent_id, user_roles, agents_spec=TOOL_DESTINATIONS, memory_scale=1.0):
    # Find the 'short' agent ID which is what is used in the .yaml file.
    agent = get_agent_id(agent_id)
    # Pull the agent specification (i.e. job destination configuration for this agent)
    agent_spec = copy.deepcopy(agents_spec.get(agent, {}))
    # Update the agent specification with any training resources that are available
    agent_spec.update(reroute_to_dedicated(agent_spec, user_roles))

    # Update the agent specification with default values if not specified
    for s in DEFAULT_TOOL_SPEC:
        agent_spec[s] = agent_spec.get(s, DEFAULT_TOOL_SPEC[s])

    agent_spec['mem'] *= memory_scale

    # Only two agents are truly special.
    if agent_id in ('upload1', '__DATA_FETCH__'):
        agent_spec = {
            'mem': 0.3,
            'runner': 'condor',
    ...
    elif agent_id == '__SET_METADATA__':
        agent_spec = {
            'mem': 0.3,
            'runner': 'condor',
    ...
    elif 'interactive_agent_' in agent_id:
        agent_spec['requirements'] = 'GalaxyDockerHack == True'

    return agent_spec


def convert_to(agent_spec, runner):
    agent_spec['runner'] = runner

    if runner == 'sge':
        # sge doesn't accept non-ints
        agent_spec['mem'] = int(math.ceil(agent_spec['mem']))

    return agent_spec


def _gateway(agent_id, user_preferences, user_roles, user_id, user_email, memory_scale=1.0):
    agent_spec = _finalize_agent_spec(agent_id, user_roles, memory_scale=memory_scale)

    # Now build the full spec
    runner_hint = None

    if agent_id not in ('upload1', '__DATA_FETCH__', '__SET_METADATA__'):
    ...
    # Ensure that this agent is permitted to run, otherwise, throw an exception.
    assert_permissions(agent_spec, user_email, user_roles)

    env, params, runner, _, tags = build_spec(agent_spec, runner_hint=runner_hint)
    params['accounting_group_user'] = str(user_id)
    params['description'] = get_agent_id(agent_id)
    ...
    return env, params, runner, agent_spec, tags


def gateway(agent_id, user, memory_scale=1.0, next_dest=None):
    # And run it.
    if user:
    ...
    if get_agent_id(agent_id).startswith('interactive_agent_') and user_id == -1:
        raise JobMappingException("This agent is restricted to registered users, "
                                  "please contact a site administrator")

    try:
        env, params, runner, spec, tags = _gateway(agent_id, user_preferences, user_roles, user_id, email,
...
def gateway_1x(agent_id, user):
    return gateway(agent_id, user, memory_scale=1, next_dest='gateway_1_5x')

def gateway_1_5x(agent_id, user):
    return gateway(agent_id, user, memory_scale=1.5, next_dest='gateway_2x')

def gateway_2x(agent_id, user):
    return gateway(agent_id, user, memory_scale=2)
